...
from models.deform.deform_conv_func import DeformConvFunction
...
class DeformConv_d(nn.Module):
    ...
    def __init__(self, in_channels, out_channels,
                 kernel_size, stride, padding, dimension='HW', dilation=1, groups=1, deformable_groups=1,
    ...
    def forward(self, input, temp):
        ...
        if self.length == 2:
            offset = temp.new_zeros(b, 3 * self.deformable_groups * self.kernel_size[0] * self.kernel_size[1] *
                                    self.kernel_size[2], t, h, w)
...
class DeformConvPack_d(DeformConv_d):
    ...
    def __init__(self, in_channels, out_channels,
                 kernel_size, stride=1, padding=1, dimension='THW',
                 dilation=1, groups=1, deformable_groups=1, im2col_step=64, bias=True, lr_mult=0.1):
        super(DeformConvPack_d, self).__init__(in_channels, out_channels,
                                               kernel_size, stride, padding, dimension, dilation, groups,
                                               deformable_groups,
                                               im2col_step, bias)
        self.dimension = dimension
        self.length = len(dimension)
        out_channels = self.deformable_groups * self.length * self.kernel_size[0] * self.kernel_size[1] * \
                       self.kernel_size[2]
        self.conv_offset = nn.Conv3d(self.in_channels,
                                     out_channels,
                                     kernel_size=self.kernel_size,
...
if __name__ == '__main__':
    x = torch.rand([1,1,10,32,32]).cuda()
    net =DeformConvPack_d(in_channels=1,out_channels=32,kernel_size=3,stride=1, padding=1, dimension='HW').cuda()
    print(net(x).shape)
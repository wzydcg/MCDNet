...
class DCN(object):
    def __init__(self, input_shape, kernel_size, extend_scope, morph, device):
        ...
        self.alphax = torch.nn.Parameter(torch.Tensor(1))
        self.alphay = torch.nn.Parameter(torch.Tensor(1))
        self.alphaz = torch.nn.Parameter(torch.Tensor(1))
    ...
    def _coordinate_map_3D(self, offset, if_offset):
        # offset
        init_rate(self.alphax)
        init_rate(self.alphay)
        init_rate(self.alphaz)
        self.alphax = self.alphax.to(self.device)
        self.alphay = self.alphay.to(self.device)
        self.alphaz = self.alphaz.to(self.device)
        ...
        if self.morph == 0:
            ...
            z_offset1 = z_offset1.to(self.device)
            y_offset1 = y_offset1.to(self.device)
            ...
            if if_offset:
                ...
                for index in range(1, center + 1):
                    z_offset1_new[center + index] = z_offset1_new[center + index - 1] + self.alphax * z_offset1[center + index]
                    z_offset1_new[center - index] = z_offset1_new[center - index + 1] + self.alphax * z_offset1[center - index]
                    y_offset1_new[center + index] = y_offset1_new[center + index - 1] + self.alphax * y_offset1[center + index]
                    y_offset1_new[center - index] = y_offset1_new[center - index + 1] + self.alphax * y_offset1[center - index]
            ...
            if if_offset:
                ...
                for index in range(1, center + 1):
                    x_offset1_new[center + index] = x_offset1_new[center + index - 1] + self.alphay * x_offset1[center + index]
                    x_offset1_new[center - index] = x_offset1_new[center - index + 1] + self.alphay * x_offset1[center - index]
                    z_offset2_new[center + index] = z_offset2_new[center + index - 1] + self.alphay * z_offset2[center + index]
                    z_offset2_new[center - index] = z_offset2_new[center - index + 1] + self.alphay * z_offset2[center - index]
            ...
            if if_offset:
                ...
                x_offset2_new[center] = 0
                for index in range(1, center + 1):
                    x_offset2_new[center + index] = x_offset2_new[center + index - 1] + self.alphaz * x_offset2[center + index]
                    x_offset2_new[center - index] = x_offset2_new[center - index + 1] + self.alphaz * x_offset2[center - index]
                    y_offset2_new[center + index] = y_offset2_new[center + index - 1] + self.alphaz * y_offset2[center + index]
                    y_offset2_new[center - index] = y_offset2_new[center - index + 1] + self.alphaz * y_offset2[center - index]
